...
# First, save A_hat, G_hat
# s_hat, industry expenditures, etc.
...
pm <- predicted_matrices(R=R,N=N,fit=fit,nonzero_vars=nonzero_vars)
A_hat <- pm$A_hat
G_hat <- pm$G_hat
rmgc(pm)

prhs <- predicted_rhs(R=R,N=N,fit=fit)
...
ggplot() + geom_point(aes(
    x=str_c("c_",var) %>% as.name() %>% eval() %>% log(),
    y=prhs[[str_c("c_",var,"_hat")]] %>% log()
  ))

# what left? need to work on the industry thing later. work on sparsity too.

# plot something...?

# c_mc_hat <- prhs$c_mc_hat
# c_a_hat <- prhs$c_a_hat
# c_g_hat <- prhs$c_g_hat
# c_ind_hat <- prhs$c_ind_hat
# rmgc(prhs)
...
## What to do next; 
# Analysis: do firm sizes match, does other stuff match, etc etc.


# 1. save firm sizes in df.

# Specificity, sensitivity.
sensitivity_specificity <- sensitivity_specificity(G,G_hat) # input true and estimated matrices.
print(sensitivity_specificity)

# Plotting: graphs, etc.
# Testing: ?

# ok, now what else? not super useful for actual data, since we have nothing to compare to.
